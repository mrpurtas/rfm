...
for col in date_list:
    df[col] = pd.to_datetime(df[col])
           ...
           df.groupby("order_channel").agg({"master_id": "count", "total_order_number": "mean", "total_order_price": "mean"})
           # 6. En fazla kazancı getiren ilk 10 müşteriyi sıralayınız.
    df.groupby("master_id").agg({'total_order_price': "sum"}).sort_values(by="total_order_price", ascending=False)
           # 7. En fazla siparişi veren ilk 10 müşteriyi sıralayınız.
df.groupby("master_id").agg({'total_order_value': "sum"}).sort_values(by="total_order_number", ascending=False)
...
rfm = df.groupby("master_id").agg({'last_order_date': lambda x: (today_date - x.max()).days,
                             ...
                             "total_order_price": lambda x: x.sum()})
...
rfm[rfm["segment"].isin(["loyal_customers", "champions"])].index
cust1 = pd.DataFrame(rfm[rfm["segment"].isin(["loyal_customers", "champions"])].index)

cust2 = pd.DataFrame(df[(df["interested_in_categories_12"].str.contains("KADIN")) & (df["total_order_price"] > 250)])                   # b. Erkek ve Çoçuk ürünlerinde %40'a yakın indirim planlanmaktadır. Bu indirimle ilgili kategorilerle ilgilenen geçmişte iyi müşteri olan ama uzun süredir
cust2.set_index("master_id", inplace=True)
cust22 = pd.DataFrame(cust2.index)


result = pd.concat([cust1, cust22]).drop_duplicates(keep=False)
